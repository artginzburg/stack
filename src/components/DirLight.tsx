...
export function DirLight({ previousTile }: { previousTile: PreviousTile }) {
  ...
  return (
    <group
      position-y={Math.max(camera.position.y - 200, previousTile.position.y)} // using "previous tile position y" as minimum to not move camera on game end, when the scene zooms out.
    >
      <directionalLight
        color={'hsl(0.1, 10%, 95%)'}
        intensity={1}
